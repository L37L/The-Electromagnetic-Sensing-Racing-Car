...
{
	read_AD();															//读取adc值
	Encoder_read();													//获取编码器速度
	motor_control();												//电机控制

	dis = get_total_encoder();							//路程计算
	
	dl1a_get_distance();										//TOF测距
	
//	gyroscope_open();												//陀螺仪计算角度
	
	huoer();																//霍尔检测
		
	if(podao_flag == 1)											//坡道计时
	{
		podao_time++;
		if(podao_time >= 1500)
		{
	...
	if((lhuandao_flag==1 || rhuandao_flag==1) && huandao_state==OUT)		//环岛出环计时，定时消环
	{
		huandao_time++;
		if(huandao_time >= 1000)
		{
			huandao_time=0;
		}
	}
	if(in_flag == 2)												//入库倒车计时（根据速度调整）
	{
		cnt++;
		if(cnt> 500)
		{
